...
interface ConfirmationConfig {
  ...
  icon?: ReactNode;
  contentComponent?: React.ComponentType<{ onDataChange: (data: string) => void }>;
}
...
export const ConfirmationProvider: React.FC<ConfirmationProviderProps> = ({
  ...
  const confirm = useCallback(
    (config: ConfirmationConfig = {}): Promise<{ confirmed: boolean; data: string | null }> => {
      return new Promise((resolve) => {
        setState({
          isOpen: true,
          config: {
            title: config.title || 'Confirmation',
            description: config.description || 'Are you sure you want to continue?',
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md overflow-hidden">
        ...
        <div className="flex justify-end p-4 bg-gray-50 dark:bg-gray-700 space-x-3">
          ...
          <button
            ...
            className={`${variantClasses.confirmButton} text-white rounded-lg transition focus:outline-none focus:ring-2 focus:ring-offset-2`}
          >
            {confirmText}
          </button>
        </div>
