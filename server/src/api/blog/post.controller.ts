...
export const saveOrPublishPost = async (req: Request, res: Response) => {
  ...
  if (publish === 'true') {
    ...
    requiredFields = ['title', 'timeRead', 'content', 'summary', 'metaTitle', 'metaDesc', 'metaKeywords', 'categories', 'tags'];
  }
  ...
  const categoriesToConnectOrCreate = categories.map(async (category: string) => {
    let categoryDoc = await Category.findOne({ slug: generateUniqueSlug(category) });
    if (!categoryDoc) {
      categoryDoc = await Category.create({ name: category, slug:  generateUniqueSlug(category) });
    }
